...
- **`postprocessor.py`** (~340 lines) — Postprocessing pipeline that runs after the two visitor passes. `postprocess(visitor, cull_subsumed_edges=True)` is the orchestrator (fixed pipeline order: `resolve_imports` → `contract_nonexistents` → `expand_unknowns` → `collapse_inner` → `cull_subsumed`). Each stage is a free function taking the visitor: `resolve_imports` (remap IMPORTEDITEM → real targets), `contract_nonexistents` (unresolved → `*.name` wildcards), `expand_unknowns` (wildcards → real targets, import-aware per #88), `collapse_inner` (fold lambdas / comprehensions back into their parent Node), `cull_subsumed` (drop uses edges a more specific edge already conveys, #140 — see below).
  - **Subsumption (#140)**: a uses edge `S → T` is dropped when `S` is a module and something *in its own file* also uses `T`, or when `T` is a module and `S` also uses something under it. Only modules widen, and only one end per test. Both restrictions are load-bearing: widening a class on the target side would delete constructor calls (`f → Thing` alongside `f → Thing.method`), and widening both ends at once lets an unrelated edge inside a subpackage stand in for a package's own import. Source-side widening counts only same-file members, because a package's dotted-name descendants are separate files. Gated by `cull_subsumed_edges` (CLI: `--keep-subsumed-edges`); tests that assert on *import resolution* need it off, since culling removes their observation point — hence the `callgraph_raw` fixture in `tests/test_analyzer.py`.
