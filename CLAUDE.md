...
- **`postprocessor.py`** (~340 lines) — Postprocessing pipeline that runs after the two visitor passes. `postprocess(visitor, cull_subsumed_edges=True)` is the orchestrator (fixed pipeline order: `resolve_imports` → `contract_nonexistents` → `expand_unknowns` → `collapse_inner` → `cull_subsumed`). Each stage is a free function taking the visitor: `resolve_imports` (remap IMPORTEDITEM → real targets), `contract_nonexistents` (unresolved → `*.name` wildcards), `expand_unknowns` (wildcards → real targets, import-aware per #88), `collapse_inner` (fold lambdas / comprehensions back into their parent Node), `cull_subsumed` (drop uses edges a more specific edge already conveys, #140 — see below).
  - **Subsumption (#140)**: a uses edge `S → T` is dropped when `S` is a module and something *in its own file* also uses `T`, or when `T` is a module and `S` also uses something under it. Only a module narrows, and only one end per test. Both restrictions are load-bearing: narrowing a class on the target side would delete constructor calls (`f → Thing` alongside `f → Thing.method`), and narrowing both ends at once accepts an unrelated edge inside a subpackage as evidence for a package's own import. Narrowing the source reaches only same-file members, because a package's dotted-name descendants are separate files. Gated by `cull_subsumed_edges` (CLI: `--keep-subsumed-edges`); tests that assert on *import resolution* need it off, since culling removes their observation point — hence the `callgraph_raw` fixture in `tests/test_analyzer.py`.
...
- **`visgraph.py`** (~270 lines) — `VisualGraph`: format-agnostic output graph. Filters edges (defines/uses), groups by namespace, applies coloring (HSL: hue = file, lightness = nesting depth). `Colorizer` handles the HSL assignment.
  - **Module nodes when grouped (#140)**: the cluster *is* the module, so every module node is drawn inside a cluster of its own, labelled `<module>` (CPython's name for the module-level code object), and its defines edges into that cluster are suppressed — the box already states containment. A module whose body has no uses edges either way isn't drawn *unless* it is the only node its box would hold — an empty cluster is not drawn at all (a subgraph is opened only when a node lands in it, and GraphViz discards an empty one it is handed), so the module would go missing entirely. `grouping_namespace()` is what redirects a module node into its own cluster instead of its parent's; note module Nodes have `namespace == ""` and carry the full dotted path as `name`. All of this is gated on `grouped` (and off for `module_level`, where every node is a module and relabelling them all `<module>` would rename the whole graph to one word); ungrouped output is untouched.
...
**Anonymous scopes are named in two dotted pieces** (`lambda.0`, `listcomp.1`), which makes the dotted path ambiguous to naive splitting — `get_parent_node` has to recognize a trailing `<kind>.<index>` pair rather than `rsplit(".", 1)`. Two independent places generate these names and they must agree exactly, since a lookup miss is a hard error that aborts the run: `analyze_scopes` numbers them per parent symtable, `_next_anon_scope_name` per `(parent_ns, kind)` during the walk. The numbering applies at every nesting depth — a lambda in a lambda is `lambda.0.lambda.0`. On 3.12+, PEP 709 means `symtable` reports no table for a comprehension (its *scope* is unchanged), so anything nested inside one is reported against the enclosing function and its scope is synthesized on demand; `is_inside_inlined_comprehension` gates that, deliberately narrowly, so any other mismatch still fails loudly.
