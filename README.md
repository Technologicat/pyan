...
Ordinary parameters, because on `*args` / `**kwargs` the annotation describes the *element* type while the parameter itself is a tuple or a dict. Subscripting one does resolve, though: in `def f(*items: Thing): items[0].method()`, `items[0]` is a single element and reaches `Thing.method`. Any other subscript resolves to nothing, pyan not tracking what a container holds.
