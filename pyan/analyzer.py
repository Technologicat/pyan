...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def _init_common(self, logger, namespace_constructors: Iterable[str] | None = None):
        ...
        *namespace_constructors* — see ``CallGraphVisitor.__init__``.  The
        ...
        # The graph: state dicts (nodes / defines_edges / uses_edges /
        # module_to_filename) plus the post-analysis query API. See
        # :mod:`pyan.callgraph`. The visitor exposes ``self.nodes`` etc.
        # via properties that read/write through ``self.graph``.
        self.graph = CallGraph(logger=self.logger)
    ...
    # sites (self.nodes[...] = ..., self.uses_edges = remap(...), etc.) keep
    # working unchanged. The dicts live on self.graph; these properties are
    # the access points.
    ...
    def nodes(self, value):
        self.graph.nodes = value

    @property
    def defines_edges(self):
        return self.graph.defines_edges

    @defines_edges.setter
    def defines_edges(self, value):
        self.graph.defines_edges = value

    @property
    def uses_edges(self):
        return self.graph.uses_edges

    @uses_edges.setter
    def uses_edges(self, value):
        self.graph.uses_edges = value

    @property
    def module_to_filename(self):
        return self.graph.module_to_filename

    @module_to_filename.setter
    def module_to_filename(self, value):
        self.graph.module_to_filename = value

    ###########################################################################
    # Query API — delegated to self.graph (see :mod:`pyan.callgraph`).

    def filter(self, node=None, namespace=None, max_iter=1000, direction="both"):
        return self.graph.filter(node, namespace, max_iter, direction)

    def filter_by_depth(self, max_depth):
        return self.graph.filter_by_depth(max_depth)

    def get_related_nodes(self, node=None, namespace=None, max_iter=1000, direction="both"):
        return self.graph.get_related_nodes(node, namespace, max_iter, direction)

    def find_paths(self, from_node, to_node, max_paths=10):
        return self.graph.find_paths(from_node, to_node, max_paths)

    @staticmethod
    def format_paths(paths):
        return CallGraph.format_paths(paths)
    ...
    def visit_Call(self, node):
        ...
        # :func:`pyan.recognizers.maybe_register_setattr_call` for the
    ...
    def get_node(self, namespace, name, ast_node=None, flavor=Flavor.UNSPECIFIED):
        """Get-or-create a graph Node for *namespace*.*name*.

        Delegates to :meth:`pyan.callgraph.CallGraph.get_node`, passing
        ``self.filename`` (the file currently being analyzed) as the
        fallback filename for newly-created Nodes whose namespace is not
        in ``module_to_filename``.

        See :meth:`CallGraph.get_node` for the full contract.
        """
        return self.graph.get_node(namespace, name, ast_node, flavor, default_filename=self.filename)
