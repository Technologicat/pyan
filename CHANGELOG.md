...
- **Advisory when `infer_root` may have misidentified the package root.** When inference walks up at least one package level and stops at a directory that has neither `__init__.py` nor a project-root marker (`pyproject.toml`, `setup.py`, `setup.cfg`), pyan now emits a warning suggesting `--root`. That layout is consistent with a top-level PEP 420 namespace package whose proper root is the parent directory; auto-walking further would be unsafe (it could climb into a directory full of unrelated repos), so the choice is left to the user. The `--root` help text now also mentions the namespace-package case explicitly. (#128)
